...
def get_zh_kcb_page_count() -> int:
    ...
    :rtype: int
    """
    res = requests_get(zh_sina_kcb_stock_count_url)
...
def stock_zh_kcb_spot() -> pd.DataFrame:
    ...
    for page in tqdm(range(1, page_count + 1), leave=False):
        ...
        res = requests_get(zh_sina_kcb_stock_url, params=zh_sina_stock_payload_copy)
...
def stock_zh_kcb_daily(symbol: str = "sh688399", adjust: str = "") -> pd.DataFrame:
    ...
    r = requests_get(zh_sina_kcb_stock_amount_url.format(symbol, symbol))
    ...
    if adjust == "hfq":
        res = requests_get(zh_sina_kcb_stock_hfq_url.format(symbol))
    ...
    if adjust == "qfq":
        res = requests_get(zh_sina_kcb_stock_qfq_url.format(symbol))
